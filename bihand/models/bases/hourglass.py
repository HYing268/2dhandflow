...
class Hourglass(nn.Module):
    ...
    def _hourglass_foward(self, n, x):
        ...
        up2 = F.interpolate(low3, scale_factor=2)#上采样
        out = up1 + up2
        return out
...
class HourglassBisected(nn.Module):
    ...
    def _hourglass_foward(self, n, x):
        ...
        #####low3_1      torch.Size([16, 256, 16, 16])
        ######up2_1      torch.Size([16, 256, 32, 32])

        up2_1 = F.interpolate(low3_1, scale_factor=2)
        up2_2 = F.interpolate(low3_2, scale_factor=2)

        # deconv = nn.ConvTranspose2d(in_channels=256, out_channels=256, kernel_size=4, stride=2, padding=1)
        # deconv.to(low3_1.device)
        # up2_1 = deconv(low3_1)
        # up2_2 = deconv(low3_2)
...
class NetStackedHourglass(nn.Module):
    def __init__(
        ...
        self.nclasses = nclasses#nclasses 是指关节点的类别数目。对于2D人体姿态估计任务，关节点通常是人体的关节部分，如手肘、膝盖等，因此 nclasses 代表着预测的2D热力图中每个像素点对应的类别数目，即关节点的数目。
        self.nstacks = nstacks
        self.in_planes = 64
    ...
    def forward(self, x):
        ...
        for i in range(self.nstacks):
            ...
            if i < self.nstacks - 1:
                ...
                hm_enc.append(y)
        return out, hm_enc #out存储的是每次循环中的输出预测结果（score），而hm_enc存储的是每次循环中的特征图（x或y）
